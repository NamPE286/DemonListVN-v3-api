...
export class EloService {
    private getK(count: number): number {
        if (count < 5) {
            return 40
        } else if (count < 15) {
            return 20
    ...
    calcElo(playerA: Elo, playerB: Elo): { newPlayerA: Elo, newPlayerB: Elo } {
        const kA = this.getK(playerA.matchCount)
        const kB = this.getK(playerB.matchCount)
    ...
    calcLeaderboard(players: any[]): Diff[] {
        ...
        for (let i = 0; i < data.length; i++) {
            for (let j = i + 1; j < data.length; j++) {
                const tmp = this.calcElo(data[i], data[j])
