alter table "public"."records" add column "queueNo" bigint;
...
SELECT
  ROW_NUMBER() OVER (ORDER BY r.timestamp) AS "orderNo",
  r.userid,
  r.levelid,
  r.timestamp,
  r."isChecked",
  ...
  p."supporterUntil",
  CASE
    WHEN p."supporterUntil" IS NULL THEN false
    ELSE p."supporterUntil" > NOW()
  END AS "isSupporterActive"
FROM
  public.records r
JOIN
...
UPDATE records
SET "queueNo" = RankedRecords."orderNo"
FROM RankedRecords
