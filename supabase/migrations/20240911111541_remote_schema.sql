...
alter table "public"."players" add column "exp" bigint not null default '0'::bigint;

alter table "public"."changelogs" add constraint "changelogs_levelID_fkey" FOREIGN KEY ("levelID") REFERENCES levels(id) ON UPDATE CASCADE ON DELETE CASCADE not valid;
...
with
  v_table_a as (
    select
      records.userid,
      coalesce(sum(records."dlPt"), 0) as sum,
      count(*)
    from
      records
    group by
