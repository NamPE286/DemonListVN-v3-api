alter table "public"."caseItems" add column "rarity" bigint;

alter table "public"."coupons" add column "owner" uuid;

alter table "public"."players" alter column "uid" set default extensions.uuid_generate_v4();

alter table "public"."caseItems" add constraint "caseItems_rarity_check" CHECK (((0 <= rarity) AND (rarity <= 4))) not valid;

alter table "public"."caseItems" validate constraint "caseItems_rarity_check";

alter table "public"."coupons" add constraint "coupons_owner_fkey" FOREIGN KEY (owner) REFERENCES players(uid) not valid;
