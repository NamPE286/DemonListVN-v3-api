...
-- Map Packs table (general, not specific to battle pass)
...
-- Battle Pass Map Packs - links season to map packs (similar to battlePassLevels)
...
-- Note: isPremium has been removed from this table. Premium status is now tracked via playerSubscriptions table.
-- Migration path: For existing data, run the following query to migrate isPremium users:
-- INSERT INTO "public"."playerSubscriptions" ("userID", "subscriptionId", "refId", "end")
-- SELECT bp."userID", s.id, bp."seasonId", NULL
-- FROM "public"."battlePassProgress" bp
-- CROSS JOIN "public"."subscriptions" s
-- WHERE bp."isPremium" = true AND s."type" = 'battlepass_premium';
-- Then drop the isPremium column: ALTER TABLE "public"."battlePassProgress" DROP COLUMN "isPremium";
...
-- Subscriptions table (defines subscription types)
...
-- Player Subscriptions (tracks player subscriptions, refId can point to battle pass season id)
...
-- Battle Pass Map Pack Progress (tracks progress on map packs in battle pass)
...
CREATE POLICY "Enable read access for all users" ON "public"."mapPacks" FOR SELECT USING (true);
CREATE POLICY "Enable read access for all users" ON "public"."mapPackLevels" FOR SELECT USING (true);
